...
create table alarmtype(alarm_type_id INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1, INCREMENT BY 1), label varchar(100), symbol varchar(32));
...
create table alarm(alarm_id INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1, INCREMENT BY 1), name varchar(32), alarm_type_id integer ,action_type_code varchar(5), wallet_id integer, value float,  foreign key (wallet_id) references wallet(wallet_id), foreign key (alarm_type_id) references alarmtype(alarm_type_id), foreign key (action_type_code) references actiontype(code));
...
insert into alarmtype values(NULL,'Cours superieur', 'SUP');
insert into alarmtype values(NULL,'Cours inferieur', 'INF');
