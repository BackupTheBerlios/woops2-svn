package isimarket.oldservants.alarmservant.impl;

import isimarket.model.Alarm;
import isimarket.model.AlarmType;
import isimarket.oldservants.alarmservant._AlarmServantImplBase;

public class AlarmServantImpl extends _AlarmServantImplBase {

	public void createAlarm(int _walletId, int _alarmType_id,
			String _actionTypeCode) {
		// TODO Auto-generated method stub

	}

	public void createAlarmType(int _alarmTypeId, String _label) {
		// TODO Auto-generated method stub

	}

	public void deleteAlarm(int _alarmId) {
		// TODO Auto-generated method stub

	}

	public void deleteAlarmType(int _alarmTypeId) {
		// TODO Auto-generated method stub

	}

	public Alarm getAlarm(int _walletId, int _alarmType_id,
			String _actionTypeCode) {
		// TODO Auto-generated method stub
		return null;
	}

	public Alarm[] getAlarmList(int _walletId) {
		// TODO Auto-generated method stub
		return null;
	}

	public AlarmType getAlarmType(int _alarmTypeId) {
		// TODO Auto-generated method stub
		return null;
	}

	public AlarmType[] getAlarmTypeList() {
		// TODO Auto-generated method stub
		return null;
	}

}
