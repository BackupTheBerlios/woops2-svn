...
public class RoleDaoTest extends TestConfiguration {
	 ...
	 * 
	 * PRINCIPLE Create a tmp role, save it into the database with the method to
	 * test. Then look for the database to check if this tmp role exists. To
	...
	public void testSaveOrUpdateRole() {
		...
		Role roleTmp = (Role) this.roleDao.getHibernateTemplate().load(
				Role.class, id);
	 ...
	 * 
	 * PRINCIPLE Create a tmp role, save it into the database. Then get all
	 * roles from the database with the method to test, and look if the size of
	 * the roles set got is >= 1. To finish delete this tmp role from the
	 * database.
	 */
	public void testGetAllRole() {
		...
		// Look if this role is also into the database and look if the size of
		// the set is >= 1.
		Set<Role> roles = this.roleDao.getAllRole();
		assertNotNull(roles);
	...
	public void testDeleteRole() {
		...
		Role roleTmp = (Role) this.roleDao.getHibernateTemplate().get(
				Role.class, id);
		assertNull(roleTmp);
