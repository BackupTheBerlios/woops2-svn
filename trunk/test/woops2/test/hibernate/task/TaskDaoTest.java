...
public class TaskDaoTest extends TestConfiguration {
	 ...
	 * 
	 * PRINCIPLE Create a tmp task, save it into the database with the method to test. Then look
	 * for the database to check if this tmp task exists. To finish delete this tmp task
	 * from the database.
	 */
	public void testSaveOrUpdateTask () {
		...
		Task taskTmp = (Task) this.taskDao.getHibernateTemplate().load(Task.class, id) ;
	 ...
	 * 
	 * PRINCIPLE Create a tmp task, save it into the database. Then get all activities from the
	...
	public void testGetAllTasks () {
		...
		// Look if this task is also into the database and look if the size of the set is >= 1.
	...
	public void testDeleteTask () {
		...
		Task taskTmp = (Task) this.taskDao.getHibernateTemplate().get(Task.class, id) ;
		assertNull(taskTmp) ;
